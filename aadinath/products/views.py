...
#                     'No data provided. Please send user_name, password', 
...
#             if not user_name:
#                 return error_response('user_name is required', code=status.HTTP_400_BAD_REQUEST)
#             if not password:
...
#             try:
#                     user = Admins.objects.get(user_name=user_name)
#                     data ={
#                         "id": user.id,
...
#                             logger.info(f"Admin login successful for: {user_name}")
...
#                             )
#                         else:
#                             logger.warning(f"Invalid password attempt for admin: {user_name}")
#                             return error_response(
#                                 'Invalid password. Please check your password and try again.', 
#                                 code=status.HTTP_401_UNAUTHORIZED
#                             )
#                     except Exception as pwd_error:
#                         logger.error(f"Password check error for admin {user_name}: {pwd_error}")
#                         return error_response(
...
class Products_API(BaseCRUDViewSet):
    ...
    #                     search_results = Admins.objects.filter(user_name=search_term)
    ...
    #         if Admins.objects.filter(user_name=data['user_name']).exists():
